import { TranslationType } from './en';

const es: TranslationType = {
  // Greetings
  greeting_morning: 'Buenos días 🌱',
  greeting_afternoon: 'Buenas tardes ☀️',
  greeting_evening: 'Buenas noches 🌙',

  // Home screen
  goal_reached: '¡Meta alcanzada! Gran trabajo saliendo hoy.',
  outside_time_awaits: 'Te esperan {{amount}} de tiempo al aire libre.',
  remaining_for_goal: 'Faltan {{amount}} para tu meta diaria.',
  this_week: 'Esta semana',
  today: 'hoy',
  no_sessions_title: 'Sin tiempo al aire libre aún.',
  no_sessions_sub: 'Deja el móvil a un lado o regístralo a mano.',
  todays_sessions: 'Sesiones de hoy',

  // Streaks
  streak_daily_singular: '{{count}} día de racha',
  streak_daily_plural: '{{count}} días de racha',
  streak_weekly_singular: '{{count}} semana de racha',
  streak_weekly_plural: '{{count}} semanas de racha',
  streak_separator: ' · ',

  // Progress ring inline timer
  ring_timer_start: 'toca para iniciar',
  ring_timer_tap_stop: 'toca para detener',
  ring_timer_outside: 'fuera',
  ring_timer_info: 'Toca el anillo para iniciar el temporizador manual',

  // Widget
  widget_start_outside: 'iniciar sesión\nal aire libre',
  widget_back_inside: 'de vuelta a casa',
  widget_started: 'iniciado',
  widget_loading: 'Cargando…',
  widget_open_app: 'Abre la app para actualizar',

  // Session sources
  source_health_connect: 'Actividad',
  source_gps: 'GPS',
  source_manual: 'Manual',
  source_timeline: 'Línea de tiempo',

  // Session review
  review: 'revisar',
  session_delete: 'Eliminar',
  session_delete_confirm_title: 'Eliminar sesión',
  session_delete_confirm_body: '¿Estás seguro? Esto no se puede deshacer.',
  session_delete_cancel: 'Cancelar',
  session_review_again: 'Revisar de nuevo',
  session_review_anyway: 'Revisar igual',
  session_edit_times: '✏️ Editar tiempos',
  session_edit_title: 'Editar tiempos de la sesión',
  session_edit_save: '✓ Guardar y aprobar',
  session_edit_hint: 'Guardar aprobará esta sesión automáticamente.',
  session_notes_title: 'Notas privadas',
  session_notes_placeholder: 'Añadir nota…',
  session_notes_save: 'Guardar',
  session_swipe_hint: 'Desliza a la izquierda para confirmar, derecha si estabas dentro.',
  session_rejected_snackbar: 'Marcado como dentro',
  undo: 'Deshacer',

  // Goals
  of: 'de',
  daily_goal: 'Meta diaria',
  weekly_goal: 'Meta semanal',

  // Navigation
  nav_home: 'Inicio',
  nav_history: 'Historial',
  nav_events: 'Sesiones',
  nav_goals: 'Metas',
  nav_settings: 'Ajustes',
  nav_weather_settings: 'Clima',

  // Days of week (short, Monday first)
  day_mon: 'L',
  day_tue: 'M',
  day_wed: 'X',
  day_thu: 'J',
  day_fri: 'V',
  day_sat: 'S',
  day_sun: 'D',

  // Goals screen
  goals_edit: 'Editar',
  goals_cancel: 'Cancelar',
  goals_save: 'Guardar',
  goals_quick_select: 'Selección rápida',
  goals_custom_minutes: 'Personalizado (min)',
  goals_placeholder_daily: 'ej., 40',
  goals_placeholder_weekly: 'ej., 200',
  goals_invalid_title: 'Meta inválida',
  goals_invalid_daily: 'Ingresa entre 1 y 720 minutos.',
  goals_invalid_weekly: 'Ingresa entre 1 y 5040 minutos.',
  goals_who_tip:
    'La OMS recomienda al menos 150 minutos a la semana — unos 30 minutos al día al aire libre.',

  // Events screen
  events_tab_approved: 'Aprobado',
  events_tab_standard: 'Estándar',
  events_tab_all: 'Todo',
  events_toggle_confirmed: 'Confirmado',
  events_toggle_review: 'Por revisar',
  events_toggle_rejected: 'Rechazado',
  events_none_recorded: 'Aún no hay sesiones. ¡Sal fuera!',
  events_confidence: 'Fiabilidad',
  events_not_outside: '✕ Dentro',
  events_confirm: '✓ Fuera',
  events_confirmed: '✓ Fuera',
  events_rejected: '✕ Dentro',
  events_discarded: 'Descartado',
  events_proposed: 'Propuesto',

  // History screen
  history_period_week: 'Semana',
  history_period_month: 'Mes',
  history_stat_total: 'Total',
  history_stat_avg: 'Media diaria',
  history_stat_goals_met: 'Metas logradas',
  history_axis_minutes: 'Min/día',
  history_axis_days_week: 'Día de semana',
  history_axis_days_month: 'Día del mes',
  history_no_data: 'Sin datos este periodo',
  history_legend_goal_met: 'Logrado',
  history_legend_below_goal: 'Por debajo',
  history_legend_today: 'Hoy',
  history_legend_target: 'Objetivo',

  // Settings screen
  settings_section_detection: 'Detección',
  settings_section_locations: 'Lugares',
  settings_section_reminders: 'Recordatorios',
  settings_section_language: 'Idioma',
  settings_section_about: 'Acerca de',
  settings_health_connect: 'Health Connect',
  settings_health_connect_desc: 'Pasos al aire libre y actividad',
  settings_hc_permission_missing: 'Faltan permisos',
  settings_hc_permission_title: 'Permiso Health Connect',
  settings_hc_permission_body:
    'TouchGrass lee tu conteo de pasos y sesiones de ejercicio para detectar automáticamente cuándo estás al aire libre. Un aumento sostenido en los pasos suele sugerir una caminata al aire libre, y las actividades registradas como ciclismo o running se añaden directamente a tu progreso diario. Todos los datos permanecen privados en tu dispositivo.',

  // Health Connect Rationale Screen
  hc_rationale_title: 'Detección de actividad al aire libre',
  hc_rationale_intro:
    'TouchGrass utiliza Health Connect para detectar de forma automática y privada cuándo estás al aire libre basándose en tus datos de actividad.',
  hc_rationale_steps_title: 'Pasos (Acceso de lectura)',
  hc_rationale_steps_body:
    'Al leer tu conteo de pasos, TouchGrass puede identificar automáticamente las caminatas al aire libre sin que tengas que iniciar manualmente un temporizador. Esto asegura que todo tu movimiento se capture con precisión manteniendo tu privacidad.',
  hc_rationale_exercise_title: 'Sesiones de ejercicio (Acceso de lectura)',
  hc_rationale_exercise_body:
    'Las actividades al aire libre como correr, andar en bicicleta o hacer senderismo de otras aplicaciones se sincronizan directamente como "tiempo de césped". Esto proporciona una visión completa y precisa de tu actividad al aire libre sin ningún esfuerzo manual.',
  hc_rationale_privacy:
    'TouchGrass solo lee los tiempos y tipos de actividades. Ninguna métrica de salud, asistentes o notas salen de tu dispositivo.',
  hc_rationale_button: 'Conectar con Health Connect',
  hc_rationale_cancel: 'Quizás más tarde',

  settings_hc_open_btn: 'Abrir Health Connect',
  settings_hc_failed_title: 'Error de conexión',
  settings_hc_failed_body: 'Otorga los permisos en ajustes. Todo es 100% privado.',
  settings_hc_verified_title: 'Conectado',
  settings_hc_verified_body: 'Lectura activada.',
  settings_hc_open_error_title: 'Error',
  settings_hc_open_error_body: 'No se pudo abrir. Hazlo manualmente.',
  // GPS permission error
  settings_error_title: 'Error',
  settings_error_open_settings_failed: 'No se pudieron abrir los ajustes.',
  settings_gps_permission_required_title: 'Permiso de GPS',
  settings_gps_permission_required_body:
    'Ubicación en segundo plano requerida para la detección. Tus datos de GPS jamás salen de tu móvil.',
  settings_permission_cancel: 'Cancelar',
  settings_permission_open: 'Abrir Ajustes',
  settings_permission_disable: 'Desactivar',
  settings_gps: 'Rastreo GPS',
  settings_gps_desc: 'Detectar cuando sales de casa (geofencing)',
  settings_gps_permission: 'Permiso necesario',
  settings_gps_permission_missing: 'Faltan permisos — toca para corregir',
  settings_activity_recognition: 'Detección de actividad',
  settings_activity_recognition_desc: 'Ahorra batería detectando movimiento en lugar de ubicación',
  settings_activity_recognition_permission_missing: 'Faltan permisos — toca para corregir',
  settings_location_radius: 'Radio de {{radius}}m · {{type}}',
  settings_location_indoor: 'Dentro',
  settings_location_outdoor: 'Fuera',
  settings_location_edit_title: 'Editar lugar',
  location_edit_label: 'Nombre del lugar',
  location_edit_label_placeholder: 'ej., Casa, Trabajo',
  location_edit_radius: 'Radio',
  location_edit_radius_hint: 'Distancia (25–250 metros)',
  location_edit_radius_hint_imperial: 'Distancia (25–250 yardas)',
  location_edit_type: 'Tipo',
  location_edit_error_title: 'Inválido',
  location_edit_error_label: 'Ingresa un nombre.',
  location_edit_error_save: 'No se pudo guardar.',
  location_edit_error_delete: 'No se pudo eliminar.',
  location_delete_btn: 'Eliminar lugar',
  location_delete_confirm_title: 'Eliminar',
  location_delete_confirm_body: '¿Seguro que quieres borrar este lugar?',
  settings_reminders_label: 'Recordatorios suaves',
  settings_reminders_sublabel: 'Te ayuda a salir en el mejor momento.',
  settings_reminders_count_off: 'Apagado',
  settings_reminders_count_per_day: '{{count}}/día',
  settings_notification_permission_title: 'Notificaciones',
  settings_notification_permission_missing: 'Faltan permisos',
  settings_notification_permission_body: 'Para recordarte que tomes el aire.',
  settings_catchup_label: 'Ayúdame a cumplirlo',
  settings_catchup_sublabel: 'Más avisos si te atrasas',
  settings_catchup_off: 'Apagado',
  settings_catchup_mellow: 'Suave',
  settings_catchup_medium: 'Medio',
  settings_catchup_aggressive: 'Fuerte',
  settings_background_tracking_label: 'Notificación de rastreo',
  settings_background_tracking_sublabel: 'Desactívala en Ajustes de Android.',
  settings_app_sublabel: 'Tu compañero para salir',
  settings_privacy: 'Privacidad',
  settings_privacy_sublabel: 'Cero rastreo. Tus datos son tuyos.',
  settings_privacy_hint: 'Lee nuestra política de privacidad',
  settings_clear_data: 'Borrar todo',
  settings_clear_data_sublabel: 'Eliminar todo permanentemente',
  settings_clear_data_confirm_title: 'Borrar datos',
  settings_clear_data_confirm_body: 'Esto destruirá tus datos. Sin copias de seguridad.',
  settings_clear_cancel: 'Cancelar',
  settings_clear_delete: 'Eliminar',
  settings_clear_data_success_title: 'Éxito',
  settings_clear_data_success_body: 'Todo limpio.',
  settings_clear_data_error_title: 'Error',
  settings_clear_data_error_body: 'Ocurrió un error.',
  settings_rerun_tutorial: 'Repetir tutorial',
  settings_rerun_tutorial_sublabel: 'Ver la guía de nuevo',

  // Appearance (dark mode)
  settings_section_appearance: 'Apariencia',
  settings_theme_label: 'Tema',
  settings_theme_sublabel: 'Elige el esquema de colores',
  settings_theme_system: 'Sistema',
  settings_theme_light: 'Claro',
  settings_theme_dark: 'Oscuro',

  // Known locations management
  nav_known_locations: 'Lugares',
  settings_locations_manage: 'Gestionar lugares',
  settings_locations_manage_desc: 'Configura las zonas donde estás bajo techo',
  settings_locations_suggestions_enabled: 'Sugerir lugares',
  settings_locations_suggestions_desc: 'Detección automática',
  settings_locations_section_suggested: 'Sugeridos',
  settings_locations_section_active: 'Activos',
  settings_location_approve: 'Aprobar',
  settings_location_deny: 'Denegar',
  settings_location_suggested_badge: 'Pendiente',
  settings_location_no_suggestions: 'Sin sugerencias',
  settings_location_no_suggestions_hint: 'Se sugiere tras pasar 2+ horas en un lugar.',
  settings_location_no_active: 'Ninguno activo',
  settings_location_no_active_hint: 'Añade tu casa u oficina.',
  settings_locations_count: '{{count}} activos',
  settings_location_deny_title: 'Denegar',
  settings_location_deny_body: 'Se ignorará.',
  settings_location_deny_confirm: 'Quitar',
  settings_location_deny_cancel: 'Cancelar',
  location_add_title: 'Añadir',
  location_suggested_label: 'Lugar sugerido',
  location_edit_address: 'Dirección cercana',
  location_edit_address_unavailable: 'No disponible',
  location_edit_address_search_placeholder: 'Buscar…',
  location_edit_address_no_results: 'No encontrado',
  location_edit_approve_title: 'Aprobar',
  location_edit_approve_confirm: 'Guardar',
  location_position_error_title: 'Ubicación inaccesible',
  location_position_error_body: 'Activa el GPS.',
  notif_location_suggestion_title: '📍 Nuevo lugar',
  notif_location_suggestion_body: 'Se detectó un sitio frecuente en tu móvil.',

  manual_title: 'Anotar tiempo al aire libre',
  manual_tab_log: '📝 Sesión pasada',
  manual_tab_timer: '⏱ Temporizador',
  manual_start_time: 'Inicio',
  manual_end_time: 'Fin',
  manual_preview: 'Vista previa',
  manual_log_btn: '✓ Guardar',
  manual_timer_ready: 'Toca iniciar y deja el móvil',
  manual_timer_running: 'Corriendo — ¡ve a tomar el aire! 🌿',
  manual_timer_start: '🌿 Salgo fuera',
  manual_timer_stop: '✓ Ya volví',
  manual_timer_cancel: 'Cancelar',
  manual_timer_stopped_hint: 'Pausado — revisa tus tiempos.',
  manual_invalid_title: 'Hora inválida',
  manual_invalid_body: 'Máximo 12 horas permitidas.',

  // Migration 1.8.0
  migration_180_title: 'TouchGrass 1.8.0',
  migration_180_body:
    'Hemos reconstruido por completo cómo TouchGrass rastrea tu tiempo al aire libre. ¡Ahora es mucho más eficiente con la batería y preciso!',
  migration_180_battery_title: 'Reactivar la optimización de batería',
  migration_180_battery_body:
    'Debido a que el nuevo rastreo es tan eficiente, ya no necesitas desactivar la optimización de batería para TouchGrass. ¡Puedes volver a activarla de forma segura para ahorrar batería!',
  migration_180_battery_button: 'Abrir configuración de batería',
  migration_180_continue_button: 'Continuar',

  // Intro/Onboarding
  intro_skip: 'Omitir',
  intro_next: 'Siguiente',
  intro_get_started: 'Empezar',
  intro_welcome_title: 'Bienvenido a TouchGrass',
  intro_welcome_body: 'Tu refugio privado para salir al aire libre.',
  intro_welcome_feature_1: 'Mide tiempo al aire libre 100% local',
  intro_welcome_feature_2: 'Fíjate metas de descanso visual',
  intro_welcome_feature_3: 'Avisos suaves y privados',
  intro_welcome_feature_4: 'Tus datos no van a la nube',
  intro_privacy_policy: 'Privacidad',
  intro_hc_title: 'Health Connect',
  intro_hc_body:
    'TouchGrass usa Health Connect para detectar privadamente actividad al aire libre a partir de tus pasos y sesiones de ejercicio.',
  intro_hc_why_title: '¿Para qué?',
  intro_hc_why_body:
    'Muchos pasos en un lugar normalmente indican que has salido a caminar. Ejercicios como correr o ir en bici también se consideran actividad al aire libre. Con GPS activado, se pueden filtrar las sesiones de bicicleta estática.',
  intro_hc_hint: 'Lo puedes activar luego.',
  intro_hc_button: 'Conectar',
  intro_hc_button_granted: 'Conectado ✓',
  intro_ar_title: 'Ahorro de batería',
  intro_ar_body:
    'Para ahorrar batería, TouchGrass solo comprueba tu ubicación cuando realmente te estás moviendo.',
  intro_ar_why_title: '¿Por qué lo necesitamos?',
  intro_ar_why_body:
    'La detección de actividad despierta la app solo cuando empiezas o dejas de moverte (caminar, correr, conducir).',
  intro_ar_button: 'Activar ahorro de batería',
  intro_ar_button_granted: 'Activado ✓',
  intro_location_title: 'Ubicación',
  intro_location_body: 'Geofencing estricto.',
  intro_location_why_title: '¿Para qué?',
  intro_location_why_body:
    'Para saber que has salido de casa sin rastrearte en un mapa de servidor.',
  intro_location_hint: 'Permite "Todo el tiempo" para esto.',
  intro_location_button: 'Permitir',
  intro_location_button_granted: 'Permitido ✓',
  intro_location_known_title: 'Precisión',
  intro_location_known_body: 'Añadir tu casa ayuda a saber cuándo sales de ella.',
  intro_location_known_set_home: 'Casa',
  intro_location_known_set_work: 'Trabajo',
  intro_location_known_set_home_done: '🏠 Casa ✓',
  intro_location_known_set_work_done: '🏢 Trabajo ✓',
  intro_location_known_hint: 'Modifícalo en Ajustes.',
  intro_notifications_title: 'Avisos',
  intro_notifications_body: 'Un pequeño toque para despejar la mente.',
  intro_notifications_why_title: '¿Para qué?',
  intro_notifications_why_body: 'Aprende de ti para no molestar.',
  intro_notifications_hint: 'Opcional.',
  intro_notifications_button: 'Activar',
  intro_notifications_button_granted: 'Activado ✓',
  intro_calendar_title: 'Calendario',
  intro_calendar_body: 'No te molestamos si estás ocupado.',
  intro_calendar_why_title: '¿Para qué?',
  intro_calendar_why_body: 'Lectura en dispositivo. No guardamos detalles.',
  intro_calendar_data_scope: 'Todo es privado.',
  intro_calendar_hint: 'Configúralo luego.',
  intro_calendar_button: 'Conectar',
  intro_calendar_button_granted: 'Conectado ✓',
  intro_calendar_buffer_label: 'Margen de reuniones',
  intro_calendar_buffer_desc: 'Sin avisos cerca de eventos',
  intro_calendar_duration_label: 'Bloquear calendario',
  intro_calendar_duration_desc: 'Añade tu tiempo al aire libre a la agenda',
  intro_ready_title: '¡Todo listo!',
  intro_ready_body: 'Es hora de salir al aire libre.',
  intro_ready_tip_title: 'Consejo',
  intro_ready_tip_body: 'Registra a mano si dejas el móvil en casa.',
  intro_ready_checklist_title: 'Checklist',
  intro_ready_checklist_item_hc: 'Conectar Health Connect.',
  intro_ready_checklist_item_ar: 'Activar detección de actividad.',
  intro_ready_checklist_item_gps: 'GPS.',
  intro_ready_checklist_item_notifications: 'Avisos.',
  intro_ready_checklist_item_calendar: 'Calendario.',
  intro_ready_widget_title: 'Widget',
  intro_ready_widget_body: 'Controla todo desde tu pantalla de inicio.',

  // Notification titles (randomly picked)
  notif_title_1: '🌿 ¡A salir!',
  notif_title_2: '☀️ Toma un poco de aire',
  notif_title_3: '🌱 Respira fuera un rato',
  notif_title_4: '👟 ¡Sal afuera!',
  notif_title_5: '🌳 La naturaleza te espera',

  // Notification bodies
  notif_body_none: 'Aún no has salido. ¡Sal a estirar las piernas!',
  notif_body_generic: '¡Sal y registra algo de tiempo!',
  notif_body_start: '¡Es hora de salir!',
  notif_body_early: '¡Estás progresando, sigue así!',
  notif_body_progress_halfway: '¡Ya casi llegas!',
  notif_body_progress_almost: '¡Solo un poco más para alcanzar tu meta!',
  notif_body_halfway: 'Faltan {{remaining}} minutos al aire libre.',
  notif_body_almost: '¡Casi! Solo {{remaining}} minutos más.',
  notif_body_done: '¡Meta lograda! ¿Otro paseo al aire libre? 🌿',

  // Reminder reason descriptions ("Why this time?" explanations appended to notification body)
  notif_reason_pattern: 'sueles salir ahora',
  notif_reason_lunch: 'perfecto para comer al aire libre',
  notif_reason_after_work: 'despeja la mente tras el trabajo',
  notif_reason_urgent: 'aún no llegas a tu meta',
  notif_reason_acted: 'otras veces has salido a esta hora',
  notif_reason_more_often: 'pediste que te avisáramos',

  // Streak encouragement for notifications
  notif_streak_daily: '¡Mantén tu racha de {{count}} días!',
  notif_streak_weekly: '¡Mantén tu racha de {{count}} semanas!',
  notif_streak_daily_at_risk: '¡No pierdas tu racha de {{count}} días!',
  notif_streak_weekly_at_risk: '¡No pierdas tu racha de {{count}} semanas!',

  // Connector word used between two reminder reason descriptions ("X, and Y.")
  notif_contributor_and: 'y',
  // Weather context appended to notification body when no contributor reasons are available
  // {{desc}} = weather description (e.g. "Sunny"), {{temp}} = formatted temperature (e.g. "22°C")
  notif_weather_context: '{{desc}}, {{temp}} fuera.',

  // Notification action buttons
  notif_action_went_outside: '✅ ¡Ya salí!',
  notif_action_snooze: '⏰ Luego',
  notif_action_less_often: '🔕 Menos',

  // Notification action confirmation messages (shown briefly after tapping an action button)
  notif_confirm_title: 'TouchGrass',
  notif_confirm_went_outside: '✅ ¡Genial! Disfruta.',
  notif_confirm_snoozed: '⏰ Te aviso en 30 minutos.',
  notif_confirm_less_often: '🔕 Entendido, te avisaré menos.',

  // Reminder feedback modal explanation messages
  notif_feedback_dismiss: 'Entendido',
  notif_feedback_went_outside_detail: 'Avisaremos más a las {{time}}.',
  notif_feedback_snoozed_detail:
    'En 30 mins, y luego menos a las {{time}} y más a las {{snoozeTime}}.',
  notif_feedback_less_often_detail: 'Avisaremos menos a las {{time}}.',

  // Less-often granular feedback modal
  notif_less_often_title: '¿Qué prefieres?',
  notif_less_often_bad_time: '🕐 Mal momento',
  notif_less_often_fewer_reminders: '🔕 Menos avisos',
  notif_fewer_reminders_confirm: 'Vale, {{newCount}} aviso(s) diarios en vez de {{oldCount}}.',
  notif_fewer_reminders_confirm_generic: 'Vale, enviaré menos avisos.',

  // Weather settings
  settings_weather_title: 'Clima',
  settings_weather_enabled: 'Avisos según el clima',
  settings_weather_enabled_desc: 'El clima afecta cuándo te avisamos',
  settings_weather_more: 'Más ajustes de clima',
  settings_weather_more_desc: 'Temperatura, lluvia, UV',
  settings_weather_permission_title: 'Permiso de Ubicación',
  settings_weather_permission_missing: 'Faltan permisos',
  settings_weather_location_permission_missing:
    'Ubicación aproximada para ver si hace buen tiempo. No se guarda.',
  settings_weather_location_request: 'Conceder',
  settings_temp_preference: 'Preferencia',
  settings_temp_cold: 'Prefiero el frío',
  settings_temp_moderate: 'Moderado',
  settings_temp_hot: 'Prefiero el calor',
  settings_weather_avoid_rain: 'Evitar la lluvia',
  settings_weather_avoid_heat: 'Evitar calor extremo',
  settings_weather_consider_uv: 'Revisar índice UV',
  settings_weather_current: 'Actual',
  settings_weather_unavailable: 'No disponible',
  settings_weather_error: 'Error al cargar',
  settings_weather_refresh: 'Actualizar',

  // Weather conditions
  weather_clear_sky: 'Despejado',
  weather_mainly_clear: 'Mayormente despejado',
  weather_partly_cloudy: 'Parcialmente nublado',
  weather_overcast: 'Nublado',
  weather_foggy: 'Niebla',
  weather_drizzle: 'Llovizna',
  weather_rain: 'Lluvia',
  weather_snow: 'Nieve',
  weather_rain_showers: 'Chubascos',
  weather_snow_showers: 'Chubascos de nieve',
  weather_thunderstorm: 'Tormenta',
  weather_unknown: 'Desconocido',
  weather_temp_unknown: 'Temperatura desconocida',

  // Notification channel
  notif_channel_name: 'Avisos al aire libre',
  notif_channel_background_name: 'Rastreo de fondo',
  notif_channel_background_desc: 'Se muestra cuando el geofencing está activo.',
  gps_tracking_notif_body: 'Detección de salida activa en segundo plano',
  notif_channel_scheduled_name: 'Programados',
  notif_channel_scheduled_desc: 'Avisos que tú has creado',
  notif_scheduled_title: 'TouchGrass',
  notif_channel_daily_planner_name: 'Planificador diario',
  notif_channel_daily_planner_desc: 'Cálculo silencioso de avisos.',
  notif_daily_planner_title: 'TouchGrass',
  notif_daily_planner_body: 'Abre la app para planear tu día.',

  // Scheduled notifications
  scheduled_notif_body: 'Momento programado para salir.',
  scheduled_empty: 'Nada programado',
  scheduled_empty_hint: 'Toca el + para añadir un recordatorio',
  scheduled_add: 'Añadir',
  scheduled_add_title: 'Nuevo aviso',
  scheduled_edit: 'Editar',
  scheduled_edit_title: 'Editar aviso',
  scheduled_delete: 'Eliminar',
  scheduled_delete_confirm_title: 'Eliminar',
  scheduled_delete_confirm_body: '¿Seguro que quieres eliminarlo?',
  scheduled_delete_cancel: 'Cancelar',
  scheduled_delete_confirm: 'Eliminar',
  scheduled_cancel: 'Cancelar',
  scheduled_save: 'Guardar',
  scheduled_time: 'Hora',
  scheduled_days: 'Días',
  scheduled_select_all: 'Todos',
  scheduled_label: 'Nombre (opcional)',
  scheduled_label_placeholder: 'ej., Paseo corto',
  scheduled_all_days: 'Cada día',
  scheduled_weekdays: 'Laborables',
  scheduled_error_title: 'Error',
  scheduled_error_no_days: 'Selecciona al menos un día',
  settings_scheduled_reminders: 'Avisos programados',
  settings_scheduled_reminders_sublabel: 'Horas fijas para descansar la vista',

  // Calendar integration
  settings_section_calendar: 'Calendario',
  settings_calendar_integration: 'Integración',
  settings_calendar_integration_desc: 'Revisión para no molestar en reuniones',
  settings_calendar_permission_title: 'Permiso',
  settings_calendar_permission_missing: 'Falta permiso',
  settings_calendar_permission_body: 'Lectura privada para silenciar avisos ocupados.',
  settings_calendar_permission_open: 'Ajustes',
  settings_calendar_permission_cancel: 'Cancelar',
  settings_calendar_buffer: 'Margen',
  settings_calendar_buffer_desc: 'Silencio antes y después de eventos',
  settings_calendar_buffer_minutes: '{{minutes}} mins',
  settings_calendar_duration: 'Bloquear calendario',
  settings_calendar_duration_desc: 'Añade la desconexión a tu agenda',
  settings_calendar_duration_off: 'Apagado',
  settings_calendar_duration_minutes: '{{minutes}} mins',
  settings_calendar_select: 'Guardar en',
  settings_calendar_select_desc: 'Destino del bloque',
  settings_calendar_select_title: 'Selecciona',
  settings_calendar_select_touchgrass: 'TouchGrass (Local)',
  calendar_event_title: '🌿 Desconexión',
  calendar_event_notes: 'Añadido por TouchGrass',
  calendar_touchgrass_name: 'TouchGrass',

  // About TouchGrass screen
  nav_about_app: 'Acerca de',
  about_intro_title: '¿Qué es?',
  about_intro_body:
    'Tu refugio privado para salir al aire libre. Medimos tu tiempo fuera y te animamos a salir.',
  about_detection_title: 'Cómo se rastrea el tiempo al aire libre',
  about_detection_body:
    'TouchGrass utiliza métodos de detección local para ayudarte a concentrarte en estar fuera :\n\n• Health Connect — Detecta automáticamente la actividad al aire libre leyendo tu conteo de pasos y sesiones de ejercicio. Un conteo de pasos alto suele indicar una caminata al aire libre, mientras que las actividades como correr o andar en bicicleta se acreditan directamente como tiempo al aire libre. Este método prioriza la privacidad y es eficiente con la batería.\n• GPS — Detecta cuando sales de una ubicación conocida (como casa o el trabajo) mediante geovallas locales.\n\nPuedes gestionar esto en Ajustes → Detección.',
  about_goals_title: 'Metas',
  about_goals_body: 'Crea tu racha y síguela en tu propio teléfono.',
  about_reminders_title: 'Avisos inteligentes',
  about_reminders_body: 'La app aprende qué momento es mejor sin molestar.',
  about_manual_title: 'Sin el móvil',
  about_manual_body: '¿Dejaste el teléfono en casa? Regístralo luego a mano.',
  about_widget_title: 'Widget',
  about_widget_body: 'Echa un vistazo a tu racha desde inicio.',
  about_privacy_title: 'Privacidad absoluta',
  about_privacy_body: 'Nada sale de aquí. Cero servidores externos, cero rastreo.',

  // Feedback & Support screen
  nav_feedback_support: 'Soporte',
  feedback_send_feedback: 'Enviar feedback',
  feedback_send_feedback_sublabel: 'Ideas o bugs',
  feedback_support_kofi: 'Apoyar al creador',
  feedback_support_kofi_sublabel: 'Invítame a un café ☕',
  feedback_google_disclosure: 'Enviado por Google Forms. Ver Privacidad.',
  settings_feedback_support: 'Soporte y Feedback',
  settings_feedback_support_sublabel: 'Contacta o apoya',

  // Error boundary crash screen
  error_boundary_title: 'Algo falló',
  error_boundary_subtitle: 'La app se cerró inesperadamente.',
  error_boundary_restart: 'Reiniciar',
  error_boundary_report: 'Reportar',

  // Background Task
  background_task_title: 'Avisos en curso',
  background_task_desc: 'Cálculo del mejor momento.',

  // Activity Log screen
  nav_activity_log: 'Registro',
  settings_section_activity_log: 'Transparence',
  settings_section_activity_log: 'Transparencia',
  settings_activity_log_sublabel: 'Lo que pasa a nivel interno',
  activity_log_empty: 'Vacío.',
  activity_log_section_hc: 'Health',
  activity_log_section_gps: 'GPS',
  activity_log_section_reminders: 'Avisos',

  // Battery optimization (Android intro)
  intro_battery_title: 'Batería',
  intro_battery_body: 'Para que los avisos no fallen, Android no debe dormirlos.',
  intro_battery_why_title: '¿Por qué?',
  intro_battery_why_body: 'La app es muy ligera, pero el sistema puede bloquearla.',
  intro_battery_button: 'Ajustes',
  intro_battery_button_done: 'Hecho ✓',
  intro_battery_hint: 'Elige "Sin restricciones".',

  // Settings battery optimization
  settings_battery_optimization: 'Optimización',
  settings_battery_optimization_sublabel:
    'Ayuda a los avisos: Ajustes → Batería → TouchGrass → Sin restricciones.',

  // Intro checklist
  intro_ready_checklist_item_battery: 'Batería sin restricciones.',

  // Time/date formatting locale tag
  locale_tag: 'es-ES',

  // Session description notes (stored with each session to describe how it was detected)
  session_notes_manual: 'Manual.',
  // GPS descriptions — {{dist}} = numeric distance, {{distUnit}} = "km" or "mi",
  //                    {{speed}} = numeric speed,   {{speedUnit}} = "km/h" or "mph"
  session_notes_gps_left_returned:
    'GPS: Salida {{start}}, retorno {{dist}} {{distUnit}} ({{speed}} {{speedUnit}}).',
  session_notes_gps_left_went:
    'GPS: Salida {{start}} a {{end}}, {{dist}} {{distUnit}} ({{speed}} {{speedUnit}}).',
  session_notes_gps_left: 'GPS: Salida {{start}}, {{dist}} {{distUnit}} ({{speed}} {{speedUnit}}).',
  session_notes_gps_returned:
    'GPS: Retorno {{end}}, {{dist}} {{distUnit}} ({{speed}} {{speedUnit}}).',
  session_notes_gps_no_location: 'Detección GPS: {{dist}} {{distUnit}} a {{speed}} {{speedUnit}}.',
  location_unknown: 'Ubicación desconocida',
  location_somewhere: 'algún lugar',
  gps_session_notes_template:
    'Sesión al aire libre: Salió de {{startLabel}} y regresó a {{regionName}}.',
  dwell_prompt_title: 'Llevas un tiempo aquí',
  dwell_prompt_body: '¿Quieres guardar esta ubicación?',
  // Health Connect descriptions — {{steps}} = formatted number, {{speed}} = numeric speed, {{speedUnit}} = "km/h" or "mph"
  session_notes_hc_steps: 'Health: {{steps}} pasos ({{speed}} {{speedUnit}}).',
  session_notes_hc_exercise: 'Health: {{exerciseName}}.',
  // Speed unit label for the device's measurement system (overridden to "mph" for imperial regions in code)
  unit_speed_metric: 'km/h',
  unit_speed_imperial: 'mph',
  // Exercise type names (Health Connect integer → display string)
  exercise_badminton: 'bádminton',
  exercise_baseball: 'béisbol',
  exercise_basketball: 'baloncesto',
  exercise_biking: 'ciclismo',
  exercise_cricket: 'críquet',
  exercise_american_football: 'fútbol americano',
  exercise_australian_football: 'fútbol australiano',
  exercise_frisbee: 'frisbee',
  exercise_golf: 'golf',
  exercise_handball: 'balonmano',
  exercise_hiking: 'senderismo',
  exercise_ice_hockey: 'hockey hielo',
  exercise_ice_skating: 'patinaje hielo',
  exercise_paddling: 'remo',
  exercise_paragliding: 'parapente',
  exercise_rock_climbing: 'escalada',
  exercise_roller_hockey: 'hockey patines',
  exercise_rowing: 'remo',
  exercise_rugby: 'rugby',
  exercise_running: 'correr',
  exercise_sailing: 'vela',
  exercise_scuba_diving: 'buceo',
  exercise_skating: 'patinaje',
  exercise_skiing: 'esquí',
  exercise_snowboarding: 'snowboard',
  exercise_snowshoeing: 'raquetas',
  exercise_soccer: 'fútbol',
  exercise_softball: 'sóftbol',
  exercise_surfing: 'surf',
  exercise_open_water_swimming: 'natación abierta',
  exercise_tennis: 'tenis',
  exercise_volleyball: 'voleibol',
  exercise_walking: 'caminar',
  exercise_water_polo: 'waterpolo',
  exercise_wheelchair: 'silla de ruedas',
  exercise_unknown: 'tipo {{type}}',

  // Permission issues banner (GoalsScreen / SettingsScreen)
  permission_issues_banner: 'Falta permiso: {{features}}.',

  // Diagnostic sheet
  diagnostic_title: 'Diagnóstico',
  diagnostic_environment: 'Entorno',
  diagnostic_native_version: 'Versión',
  diagnostic_launch_type: 'Inicio',
  diagnostic_runtime_version: 'Huella de runtime',
  diagnostic_update_id: 'Update ID',
  diagnostic_launch_embedded: 'Integrada',
  diagnostic_launch_ota: 'OTA',
  diagnostic_unknown: 'desconocido',
  diagnostic_none: 'ninguno',
  diagnostic_share: 'Compartir',
  diagnostic_check_update: 'Buscar versión',
  diagnostic_update_checking: 'Buscando…',
  diagnostic_update_done: 'Actual',
  // Update splash screen
  update_splash_checking: 'Buscando…',
  update_splash_downloading: 'Instalando…',
};

export default es;
