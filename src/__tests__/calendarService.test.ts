...
describe('calendarService', () => {
  ...
  describe('addOutdoorTimeToCalendar', () => {
    ...
    it('logs that fallback payload succeeded when calendar debug logging is enabled', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          calendarId: 'touchgrass-cal-id',
          calendarLabel: 'TouchGrass',
          payload: 'fallback',
        })
      );
      logSpy.mockRestore();
    ...
    it('returns false and logs full error when all stages fail', async () => {
      ...
      mockCreateEvent
        ...
        .mockRejectedValueOnce({
          code: 'E_EVENT_NOT_SAVED',
          message: 'final failure',
          nativeDescription: 'CalendarProvider rejected',
        }); // stage 4 ultra-minimal on fresh
    ...
    it('does not create duplicate event when matching slot already exists', async () => {
      ...
      mockGetEvents.mockResolvedValueOnce([
        { title: '🌿 Offline time', startDate: start.toISOString(), endDate: end.toISOString() },
      ]);
  ...
  describe('getOrCreateTouchGrassCalendar', () => {
    it('returns cached ID when the calendar still exists', async () => {
      ...
      expect(mockUpdateCalendar).toHaveBeenCalledWith('existing-tg-id', {
        isSynced: true,
        isVisible: true,
      });
    });
      ...
      mockGetCalendars.mockResolvedValueOnce([]).mockResolvedValueOnce([
        {
          ...
          allowsModifications: false,
          accessLevel: 'read',
          source: { type: 'local', isLocalAccount: true },
        },
      ]);
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
  ...
  describe('cleanupTouchGrassCalendars', () => {
    it('merges duplicate local TouchGrass calendars into one primary calendar', async () => {
      ...
      expect(mockCreateEvent).toHaveBeenCalledWith(
        'primary-id',
        expect.objectContaining({ title: '🌿 Offline time' })
      );
  ...
  describe('deleteFutureTouchGrassEvents', () => {
    ...
    it('deletes all events returned by getEventsAsync in the given window', async () => {
      ...
      mockGetEvents.mockResolvedValue([
        { id: 'ev-1', title: '🌿 Offline time' },
        { id: 'ev-2', title: '🌿 Offline time' },
      ]);

      const from = new Date('2026-03-14T08:00:00');
      await deleteFutureTouchGrassEvents(from, 3);

      expect(mockGetEvents).toHaveBeenCalledWith(['cal-123'], from, expect.any(Date));
    ...
    it('silently ignores errors when deleting individual events', async () => {
      ...
      mockGetEvents.mockResolvedValue([{ id: 'ev-gone', title: '🌿 Offline time' }]);
