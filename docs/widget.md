...
The config plugin is registered in `app.json` and automatically runs during `npx expo prebuild`.
...
The widget is implemented using an Expo config plugin (`withWidgetPlugin.js`) that automatically injects the native Android code during the prebuild process. This approach keeps the widget code in the repository while the generated `android/` directory remains gitignored.

When you run `npx expo prebuild`, the plugin:

1. Adds the widget receiver to `AndroidManifest.xml`
2. Creates `ProgressWidgetProvider.kt` in the Java source directory
3. Generates widget layout XML (`widget_progress.xml`)
4. Creates widget configuration (`widget_info.xml`)
5. Adds all drawable resources (progress rings, icons, backgrounds)
6. Adds widget description string to `strings.xml`
